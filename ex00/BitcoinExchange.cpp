...
/*   Updated: 2026/04/27 20:00:36 by dmaestro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void BitcoinExchange::setSeparator( std::string& ref) const  
{
    this->separator + ref;
}
...
void dateValue(std::string line, long long first_date, BitcoinExchange& Data, unsigned int actualLine)
{
    ...
    for(size_t i = 0; i < buff.length(); i++)
    {
        ...
        else if((i % 2 != 0  && i != 2) && !std::isdigit(buff[i]))
            Data.setError(actualLine, BAD_DATE);            
    }
...
long long dateToLongLongConverter(std::string& date,  BitcoinExchange& Data,unsigned int actuaLine, long long first_date )
{
    ...
    if(!validDay(days, month, years))
        Data.setError(actuaLine, BAD_DATE);
    result = years*1000 + month*100 + days;
    if(result < first_date)
    return(result);
    
}
void BitcoinExchange::parseDataBase(const std::string & Data)
{
    std::ifstream file;
    ...
    takeError(this->ErrorCheck.at(0));
    while(std::getline(file, actualLine))
    {
        if(actualLine.empty())
        {
